...
class Noticias{
	...
	public function __construct( $url=null, $nombre=null, $noticia=null, $fechaPub=null, $idNoticias=null) {
...
*/
	...
	public function guardar(){
		...
		if ($this->idNoticias) { /* modifica */
			$consulta = $conexion->prepare('UPDATE ' . self::TABLA . ' SET url = :url, nombre = :nombre, noticia = :noticia, fechaPub = :fechaPub WHERE idNoticias = :idNoticias'); 
		...
		$consulta = $conexion->prepare('INSERT INTO ' . self::TABLA . ' (url, nombre, noticia, fechaPub) 
			value (:url, :nombre, :noticia, :fechaPub)'); 
		    $consulta->bindParam(':url', $this->url);
			$consulta->bindParam(':nombre', $this->nombre);
			$consulta->bindParam(':noticia', $this->noticia);
			$consulta->bindParam(':fechaPub', $this->fechaPub);	
