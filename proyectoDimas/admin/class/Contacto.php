...
class Contacto{
	...
	$email=null, $url=null, $mensaje=null, $idContacto=null) {
	...
	public function guardar(){
		...
		if ($this ->idContacto) { /* modifica */
			$consulta = $conexion->prepare('UPDATE ' . self::TABLA . ' SET nombre = :nombre, apellidoPat = :apellidoPat, 
				apellidoMat = :apellidoMat, email = :email,	url = :url, mensaje = :mensaje			
		...
		url, mensaje) value (:nombre, :apellidoPat, :apellidoMat, :email, :url, :mensaje)'); 
	...
	public static function buscarPorId($idContacto) {
		...
		if ($registro) {
		 	...
		 	return new self($registro['nombre'], $registro['apellidoPat'], $registro['apellidoMat'], $registro['email'], 
		 		$registro['url'], $registro['mensaje'], $idContacto);
		 } else {
		 	return false; 
		 }
