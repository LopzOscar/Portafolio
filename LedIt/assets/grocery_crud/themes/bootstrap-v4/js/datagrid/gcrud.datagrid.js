...
(function ($) {
    ...
    Datagrid.prototype.clearFilteringCache = function () {
        ...
        this.gcrud_container.find('.grocery-crud-table tr:first')
            .find('.active')
            .removeClass('active table-active ordering-desc ordering-asc');
        this.gcrud_container.find('.grocery-crud-table tr:first').find('.fa-chevron-up,.fa-chevron-down,.el-chevron-up,.el-chevron-down').remove();
        this.gcrud_container.find('td.active').removeClass('active table-active');
    ...
    Datagrid.prototype.checkCachedData = function () {
        ...
        if (cachedDatra.order_by[0] !== null) {
            var $column_to_order = this.gcrud_container.find('.grocery-crud-table tr:first')
                ...
                .addClass('active table-active ordering-' + cachedDatra.order_by[1]);

            var arrow_class_name = cachedDatra.order_by[1] === 'asc' ? 'fa-chevron-up el-chevron-up' : 'fa-chevron-down el-chevron-down';

            $column_to_order.html("<div>" + $column_to_order.text() + "</div><i class='fa el " +  arrow_class_name + "'></i>");
    ...
    Datagrid.prototype.appendSearchClearButtons = function () {
        ...
        datagrid_object.gcrud_container.find('.grocery-crud-table input.searchable-input').each(function () {
            ...
            $(this).after('<i class="fa fa-times el el-remove clear-search"></i>');
    ...
    Datagrid.prototype.datagridOnAjaxComplete = function (result) {
        ...
        this.listenerDeleteRowClick();
        this.listenerFancybox();
    };

    Datagrid.prototype.listenerFancybox = function () {

        // In case image-thumbnail is not present then the fancybox library is not loaded either!
    ...
    Datagrid.prototype.listenerColumnWithOrdering = function () {
        ...
        $('.column-with-ordering').click(function () {
            ...
            if ($(this).hasClass('ordering-asc')) {
                ...
                $(this).html("<div>" + column_title + "</div><i class='fa el fa-chevron-down el-chevron-down'></i>");
                $(this).addClass('ordering-desc').removeClass('ordering-asc');
            } else {
                $(this).closest('.grocery-crud-table').find('th').removeClass('ordering-desc').removeClass('ordering-asc');

                $(this).html("<div>" + column_title + "</div><i class='fa el fa-chevron-up el-chevron-up'></i>");
    ...
    Datagrid.prototype.listenerSearchButton = function () {
        ...
        $('.search-button>input.search-input').change(function () {
            ...
            if ($(this).val() !== '') {
                $(this).after('<i class="fa fa-times el el-remove clear-all-search"></i>');
    ...
    Datagrid.prototype.listenerSelectRow = function () {
        ...
        datagrid_object.gcrud_container.find('.select-row').click(function () {
            if ($(this).is(':checked')) {
                $(this).closest('tr').addClass('warning table-warning');
            } else {
                $(this).closest('tr').removeClass('warning table-warning');
            }
    ...
    Datagrid.prototype.listenerDeleteMultiple = function () {
        ...
        this.gcrud_container.find('.delete-selected-button').click(function () {
            ...
            total_to_delete = this.gcrud_container.find('.select-row:checked').length;

            if (total_to_delete === 1) {
                this.gcrud_container.find('.alert-delete-multiple').addClass('hidden');
                this.gcrud_container.find('.alert-delete-multiple-one').removeClass('hidden');
            } else {
                this.gcrud_container.find('.alert-delete-multiple').removeClass('hidden');
                this.gcrud_container.find('.delete-items-amount').html('' + total_to_delete);
                this.gcrud_container.find('.alert-delete-multiple-one').addClass('hidden');
            }
